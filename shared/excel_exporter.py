...
# IMPORTAÇÕES DAS REGRAS CONTÁBEIS:
from cte.constants import CTE_ACCOUNTING_COLUMNS
from nfe.constants import NFE_ACCOUNTING_COLUMNS
...
class ExcelExporter:
    ...
    def export(self, output_filename: str):
        ...
        sheet_title = "NFe Data" if self.doc_type == DocType.NFE else "CTe Data"
    ...
    def _build_main_sheet(self, ws):
        ...
        # --- A MÁGICA DOS CABEÇALHOS DINÂMICOS ---
        # Suporta comp_ (para CT-e) e Extra_ (para tags surpresas na NF-e)
        dynamic_cols = sorted({
            key for row in self.main_data
            for key in row
            if (key.startswith("comp_") or key.startswith("Extra_")) and key not in self.main_headers
        })
            ...
            for header in final_headers:
                ...
                if header == "ide_dhEmi" and raw_val:
                    try:
                        ...
                        cell.value = raw_val
                        cell.number_format = '@'
                elif (self.doc_type == DocType.CTE and (header in CTE_ACCOUNTING_COLUMNS or header.startswith("comp_"))) or \
